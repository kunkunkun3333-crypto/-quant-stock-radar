...
with st.sidebar:
    ...
    universe_mode = st.radio('股票池模式', ['自動擴充', '示範22檔'], index=0, help='自動擴充會從公開市場清單抓取代號，再套用市值/成交量門檻。')
    max_us = st.slider('美股最多掃描檔數', 50, 1200, 300, 50)
    max_tw = st.slider('台股最多掃描檔數', 50, 1800, 300, 50)
    top_n = st.slider('顯示前 N 名', 5, 100, 30)
...
def tickers_from_upload(upload, market):
    if upload is None:
        mode = 'demo' if universe_mode == '示範22檔' else 'auto'
        limit = max_us if market == 'US' else max_tw
        return qr.get_universe(market, mode=mode, max_symbols=limit)
...
if st.button('🚀 開始掃描', type='primary', use_container_width=True):
    ...
    try:
        ...
        rows=[]
        us_list = tickers_from_upload(us_upload,'US') if '美股' in markets else []
        tw_list = tickers_from_upload(tw_upload,'TW') if '台股' in markets else []
        st.info(f'本次股票池：美股 {len(us_list)} 檔｜台股 {len(tw_list)} 檔｜合計 {len(us_list)+len(tw_list)} 檔')
        if '美股' in markets:
            progress.progress(20,text=f'掃描美股 {len(us_list)} 檔…'); rows += qr.screen_market(us_list,'US',benchmarks)
        if '台股' in markets:
            progress.progress(55,text=f'掃描台股 {len(tw_list)} 檔…'); rows += qr.screen_market(tw_list,'TW',benchmarks)
        if not rows:
